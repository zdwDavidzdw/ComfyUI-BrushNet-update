...
June 1, 2026. **ComfyUI compatibility fix (local patch).** Newer ComfyUI versions pass a `latent_shapes` argument through the `OUTER_SAMPLE` wrapper chain during KSampler. The plugin's `brushNet_out_sample_wrapper` in `model_patch.py` did not accept this parameter, causing `TypeError: brushNet_out_sample_wrapper() got an unexpected keyword argument 'latent_shapes'`. Fixed by adding `latent_shapes=None` to the wrapper signature and forwarding it to the next executor. This is a pass-through change only — BrushNet patching, sampling, and generation output are unchanged.  
**（2026-06-01 本地补丁：修复新版 ComfyUI 采样时传入 `latent_shapes` 导致的 TypeError；仅修改 `model_patch.py` 参数透传，不影响 BrushNet 功能与生成效果。）**
